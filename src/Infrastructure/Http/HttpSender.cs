...
namespace DataConnect.Infrastructure.Http;
...
public class HttpSender : IDisposable
{
    ...
    private async Task<Result<JsonObject, Error>> GetRequestAsync(HttpRequestMessage requestTemplate, HttpMethod method)
    {
        ...
        for (int attempt = 0; attempt < maxRetries; attempt++) {
            ...
            {
                // Log retries - will be handled by DI logging in the service layer
    ...
    public async Task<Result<JsonObject, Error>> SimpleAuthBodyRequestAsync(List<KeyValuePair<string, string>> payload, HttpMethod method, Features.DataExtraction.Models.ConnectionInfo connectionInfo)
    {
        ...
        var requestContent = AddRequestContent(HttpSimpleAuth(user, password, connectionInfo.Url), content);
